...
// EXTERNAL MODULE: ./node_modules/globrex/index.js
var globrex = __nccwpck_require__(617);
...
class Agent extends external_http_namespaceObject.Agent {
    constructor(opts) {
        super(opts);
        this[INTERNAL] = {};
    }
    ...
    incrementSockets(name) {
        ...
        const fakeSocket = new external_net_namespaceObject.Socket({ writable: false });
    ...
    getName(options) {
        ...
        if (secureEndpoint) {
            return external_https_namespaceObject.Agent.prototype.getName.call(this, options);
        }
    ...
    createSocket(req, options, cb) {
        ...
        Promise.resolve()
            ...
            if (socket instanceof external_http_namespaceObject.Agent) {
                try {
...
const setServernameFromNonIpHost = (options) => {
    if (options.servername === undefined &&
        options.host &&
        !external_net_namespaceObject.isIP(options.host)) {
        return {
            ...options,
...
class HttpsProxyAgent extends Agent {
    ...
    async connect(req, opts) {
        ...
        if (proxy.protocol === 'https:') {
            ...
            socket = external_tls_namespaceObject.connect(setServernameFromNonIpHost(this.connectOpts));
        }
        else {
            dist_debug('Creating `net.Socket`: %o', this.connectOpts);
            socket = external_net_namespaceObject.connect(this.connectOpts);
        }
        const headers = typeof this.proxyHeaders === 'function'
            ? this.proxyHeaders()
            : { ...this.proxyHeaders };
        const host = external_net_namespaceObject.isIPv6(opts.host) ? `[${opts.host}]` : opts.host;
        ...
        if (connect.statusCode === 200) {
            ...
            if (opts.secureEndpoint) {
                ...
                return external_tls_namespaceObject.connect({
        ...
        req.once('socket', (s) => {
            ...
            external_assert_namespaceObject(s.listenerCount('data') > 0);
...
async function main() {
  index_debug("Running size-label-action...");

  const GITHUB_TOKEN = external_process_namespaceObject.env.GITHUB_TOKEN;
  if (!GITHUB_TOKEN) {
    throw new Error("Environment variable GITHUB_TOKEN not set!");
  }

  const GITHUB_EVENT_PATH = external_process_namespaceObject.env.GITHUB_EVENT_PATH;
  ...
  index_debug("Event payload:", eventDataStr);
  ...
  const isIgnored = parseIgnored(external_process_namespaceObject.env.IGNORED);
  ...
  const proxyUrl = external_process_namespaceObject.env.HTTPS_PROXY || external_process_namespaceObject.env.https_proxy;

  const octokit = new dist_src_Octokit({
    auth: `token ${GITHUB_TOKEN}`,
    baseUrl: external_process_namespaceObject.env.GITHUB_API_URL || "https://api.github.com",
  ...
  const githubOutput = external_process_namespaceObject.env.GITHUB_OUTPUT;
  if (githubOutput) {
    (0,external_fs_namespaceObject.writeFileSync)(githubOutput, `sizeLabel="${sizeLabel}"`);
    index_debug(`Written label '${sizeLabel}' to ${githubOutput}`);
  }
  ...
  for (const label of remove) {
    index_debug("Removing label:", label);
    try {
      ...
      index_debug("Ignoring removing label error:", error);
    }
  }
...
function index_debug(...str) {
  if (external_process_namespaceObject.env.DEBUG_ACTION) {
...
async function readEventFile(path) {
  return new Promise((resolve, reject) => {
    (0,external_fs_namespaceObject.readFile)(path, { encoding: "utf8" }, (err, data) => {
      if (err) {
        reject(err);
      } else {
        resolve(data);
...
function getSizesInput() {
  let inputSizes = external_process_namespaceObject.env.INPUT_SIZES;
...
if (external_url_namespaceObject.fileURLToPath(import.meta.url) === external_process_namespaceObject.argv[1]) {
  main().then(
    () => (external_process_namespaceObject.exitCode = 0),
    e => {
      external_process_namespaceObject.exitCode = 1;
      console.error(e);
    }
  );
}
...
var __webpack_exports__parseIgnored = __webpack_exports__.s;
export { __webpack_exports__main as main, __webpack_exports__parseIgnored as parseIgnored };
